...
- [ ] **Unwrap transfer/proxy from public types** — Refine `Promisified<T>` so callers don't need `as any` casts when passing raw `ArrayBuffer` or callback arguments (both are now auto-detected, but the types still expect `Transfer<>` / `(…) => …` wrappers).
